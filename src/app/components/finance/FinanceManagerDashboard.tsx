    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h2 className="text-sm font-black text-indigo-600 uppercase tracking-widest mb-2">Panel de Control Financiero</h2>
                ...
                <div className="flex gap-3">
                    ...
                    <button className="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold text-sm hover:bg-indigo-700 shadow-xl shadow-indigo-200 transition-all">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-100 border border-gray-50 flex flex-col relative overflow-hidden">
                    ...
                    <div className="relative">
                        ...
                        <div className="space-y-5">
                            <PremiumPnLRow label="Ingresos Operacionales" amount="$450,000" type="positive" />
                            <PremiumPnLRow label="Costo de Mercancía" amount="($210,000)" type="negative" />
                            <div className="flex items-center gap-4 py-2">
                                <div className="h-px bg-gray-100 flex-1" />
                                <span className="text-[10px] font-black text-gray-300 uppercase tracking-tighter">SUBTOTAL BRUTO</span>
                                <div className="h-px bg-gray-100 flex-1" />
                            </div>
                            <PremiumPnLRow label="Utilidad Bruta" amount="$240,000" type="highlight" />
                            <PremiumPnLRow label="Gastos Administrativos" amount="($85,000)" type="negative" />
                            <PremiumPnLRow label="Impuestos de Ley" amount="($32,500)" type="negative" />
                            <div className="h-0.5 bg-gray-100 my-4" />
                            <PremiumPnLRow label="Utilidad Neta (EBITDA)" amount="$122,500" type="final" />
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl shadow-indigo-900/20 text-white flex flex-col">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <span className="px-3 py-1 bg-white/10 rounded-full text-[9px] font-black uppercase tracking-widest text-indigo-400">Queue</span>
                    </div>

                    <div className="space-y-5 flex-1 overflow-y-auto custom-scrollbar pr-2 mb-8">
                        <AuthItem type="Pago Proveedor" ref_code="INV-882" amount="$12,400" department="Inventario" />
                        <AuthItem type="Viáticos de Venta" ref_code="RG-012" amount="$450" department="Ventas" />
                        <AuthItem type="Nómina Quincenal II" ref_code="NOM-01" amount="$45,000" department="RRHH" />
                    </div>

                    <button className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm hover:bg-indigo-500 transition-all flex items-center justify-center gap-2 group active:scale-95 shadow-xl shadow-indigo-900/50">
...
function FinMetricCard({ title, value, sub, icon, color, trend }: any) {
    const accents: any = {
...
function PremiumPnLRow({ label, amount, type }: any) {
    const styles: any = {
        positive: 'text-gray-700 bg-transparent',
        negative: 'text-rose-600 bg-transparent',
        highlight: 'text-indigo-900 font-black bg-indigo-50',
        final: 'text-white bg-indigo-600 shadow-xl shadow-indigo-100',
    };

    return (
        <div className={`flex items-center justify-between p-4 rounded-2xl transition-all ${styles[type]}`}>
...
function AuthItem({ type, ref_code, amount, department }: any) {
    return (
