...
export function SalesManagerDashboard({ onNavigate }: { onNavigate?: (menu: string) => void }) {
    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h2 className="text-sm font-black text-blue-600 uppercase tracking-[0.2em] mb-3">Panel de Control de Ventas</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-100 border border-gray-50 flex flex-col">
                    ...
                    <div className="space-y-8 flex-1">
                        <PremiumSellerRow
                            name="Mario Sánchez" id="MS" sales="$45,200" quota={92} status="Top Seller"
                            onClick={() => setSelectedDetail({ type: 'seller', name: 'Mario Sánchez', id: 'MS', sales: '$45,200', quota: 92 })}
                        />
                        <PremiumSellerRow
                            name="Elena Rodríguez" id="ER" sales="$38,150" quota={76} status="On Track"
                            onClick={() => setSelectedDetail({ type: 'seller', name: 'Elena Rodríguez', id: 'ER', sales: '$38,150', quota: 76 })}
                        />
                        <PremiumSellerRow
                            name="José Pérez" id="JP" sales="$22,400" quota={45} status="Average"
                            onClick={() => setSelectedDetail({ type: 'seller', name: 'José Pérez', id: 'JP', sales: '$22,400', quota: 45 })}
                        />
                        <PremiumSellerRow
                            name="Ana Casillas" id="AC" sales="$12,800" quota={25} status="Training"
                            onClick={() => setSelectedDetail({ type: 'seller', name: 'Ana Casillas', id: 'AC', sales: '$12,800', quota: 25 })}
                        />
                    </div>
                </div>
                ...
                <div className="space-y-8">
                    <div className="bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl shadow-blue-900/10 text-white">
                        <h3 className="text-lg font-black mb-8 flex items-center justify-between">
                            ...
                            <button onClick={() => onNavigate?.('quotes')} className="text-xs font-bold text-gray-500 hover:text-white transition-colors">Ver Todas</button>
                        </h3>
                        <div className="space-y-5">
                            <QuoteItem
                                name="Corporación ABC" amount="$12,500" prob={90} color="emerald"
                                onClick={() => setSelectedDetail({ type: 'quote', name: 'Corporación ABC', amount: '$12,500', prob: 90 })}
                            />
                            <QuoteItem
                                name="Logística Valencia" amount="$8,200" prob={75} color="blue"
                                onClick={() => setSelectedDetail({ type: 'quote', name: 'Logística Valencia', amount: '$8,200', prob: 75 })}
                            />
                            <QuoteItem
                                name="Autopartes PLC" amount="$4,100" prob={50} color="amber"
                                onClick={() => setSelectedDetail({ type: 'quote', name: 'Autopartes PLC', amount: '$4,100', prob: 50 })}
                            />
                        </div>
                    </div>
...
function SalesMetricCard({ title, value, trend, icon, color, onClick }: any) {
    const colors: any = {
...
function PremiumSellerRow({ name, id, sales, quota, status, onClick }: any) {
    return (
        <div className="group cursor-pointer" onClick={onClick}>
            <div className="flex items-center gap-6 mb-3">
                ...
                <button className="p-2 text-gray-300 group-hover:text-blue-600 transition-colors"><ChevronRight size={20} /></button>
            </div>
        </div>
    );
}

function QuoteItem({ name, amount, prob, color, onClick }: any) {
    const dots: any = {
